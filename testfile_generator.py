...
@contextmanager
def testfiles(nfiles=1, max_duration=1.0, use_rand=True):
    with TemporaryDirectory() as tmpdir:

        def generate(i):
            fs = random.choice(fs_choices)
            nb_samples = max(int(fs * random.uniform(0, max_duration)), 1)
            ...
            def generate_expr(is_rand):
                if is_rand:
                    ...
                    expr = f"sin({f0}*2*PI*t)"
                return expr

            layout = random.choice(layout_choices)
            expr = "|".join((generate_expr(i % 2 if use_rand else 0) for i in range(layout[1])))
